...
| `password_hash` | TEXT NOT NULL | PBKDF2ハッシュ（crypto.subtle） |
| `role` | TEXT NOT NULL | `"user"` / `"admin"`（デフォルト: `"user"`） |
| `created_at` | INTEGER NOT NULL | Unix タイムスタンプ（秒） |
| `updated_at` | INTEGER NOT NULL | Unix タイムスタンプ（秒） |

> 管理画面は OAuth セッション Cookie（JWT）の `sub` からユーザーを特定し、
> `role = "admin"` のみアクセスを許可する。スコープではなくロールで制御する。
...
| カラム | 型 | 説明 |
|-------|-----|------|
| `id` | TEXT PK | crypto.randomUUID()（client_id として使用） |
...
| `first_party` | INTEGER NOT NULL | `1` = ファーストパーティ（自社アプリ）、`0` = サードパーティ。ファーストパーティは同意画面をスキップして自動承認される |
| `created_at` | INTEGER NOT NULL | Unix タイムスタンプ（秒） |
...
// 投入されるユーザー
users: [
  { email: "admin@example.com", password: "password", role: "admin" }  // PBKDF2ハッシュ（crypto.subtle）化
]
...
oauth_clients: [
  {
    ...
    name: "web",
    redirect_uris: [
      "http://localhost:30000/auth/callback",         // ローカル開発
      "https://web.h-kawai-tech.workers.dev/auth/callback"  // 本番
    ],
    token_endpoint_auth_method: "none",
    scopes: "read write",
    first_party: true   // 自社アプリのため同意画面をスキップ
  }
]
```
...
pnpm -F @mcp-oauth/database db:seed           # ローカル
pnpm -F @mcp-oauth/database db:seed:remote    # 本番
```
