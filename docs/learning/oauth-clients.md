...
## 2. `first_party` フラグと同意画面スキップ

`oauth_clients` テーブルには `first_party` 列（boolean）がある。

| クライアント | `first_party` | 同意画面 |
|------------|--------------|---------|
| web-client（自分たちのSPA） | `true` | **スキップ**（自動承認） |
| Claude（DCR登録） | `false` | 表示する |

`first_party = true` のクライアントは `/authorize` でログイン済みが確認できた時点で、
同意画面を出さずに認可コードを発行して `redirect_uri` へ直接リダイレクトする。

### なぜ `skip_consent` ではなく `first_party` か

`skip_consent` はある特定の動作（スキップする）を名前にしているが、
`first_party`「自分たちのアプリかどうか」という**属性**を表す。

属性で管理する利点:
- 将来「first_party クライアントには別のレート制限を適用する」などの拡張が自然にできる
- 「なぜスキップするのか」が名前から分かる（自分たちのアプリだから信頼している）
- Auth0・Okta などの業界標準の考え方と一致する
